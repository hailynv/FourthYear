/*
* TotalRainfall calculates the total rainfall in the
* dataset by ZIP code and year. 
*
* @author  Bryan A. Lugo, Hailyn Valle Luque
*/
...
public class TotalRainfall extends Configured implements Tool {
	...
	public static class MyMapper extends Mapper<LongWritable, Text, Text, DoubleWritable> {
		...
		public void map(LongWritable inputKey, Text inputValue, Context context)
			...
			context.write(new Text(element[ZIPCODE] + "\t" + element[YEAR]),
					new DoubleWritable(Double.parseDouble(element[PERCIP])));
	...
	public static class MyReducer extends Reducer<Text, DoubleWritable, Text, Text> {
		...
		public void reduce(Text key, Iterable<DoubleWritable> values, Context context)
			...
			while (valItr.hasNext()) {
				totalRain += valItr.next().get();
			}
			context.write(key, new Text(totalRain + ""));
	...
	public int run(String[] args) throws Exception {
		...
		if (args.length != 3) {
			System.out.println(
					"Usage: Project1.jar TotalRainfall <input directory> <ouput directory> <number of reduces>");
		...
		Job job = new Job(conf, "Total Rainfall by ZIP and Year");
